...
\caption{Merge Gaps}
\begin{algorithmic}
\FOR{$gap$ in $pairwise\_center$ or $cluster\_center$}
    \IF{$gap$ in $pairwise\_center$ and $gap$ not in $cluster\_center$}
        \FOR{$sequence$ in $cluster$}
            \STATE $sequence$ = $sequence$.insert($gap$, $position$)
        \ENDFOR
    \ELSIF{$gap$ not in $pairwise\_center$ and $gap$ in $cluster\_center$}
